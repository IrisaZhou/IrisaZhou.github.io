...
async def _(mo, pd):
    ...
    if sys.platform == "emscripten":
        ...
        data_url = mo.notebook_location() / "public" / "FYGDP.csv"
        response = await pyfetch(str(data_url))
        response.raise_for_status()
        gdp = pd.read_csv(
            StringIO(await response.string()),
            parse_dates=["observation_date"],
        )
    else:
        data_path = Path(__file__).resolve().parent / "public" / "FYGDP.csv"
        gdp = pd.read_csv(data_path, parse_dates=["observation_date"])
